...
class Unzipper {
    ...
    tailrec fun decompress(currentString: String, length: Long = 0): Long {
        val nextMarkerStart = currentString.indexOf("(")
        if (nextMarkerStart == -1) return length + currentString.length

        val nextMarkerEnd = currentString.indexOfFirst { it == ')' }
        val markerLength = currentString.substring(nextMarkerStart + 1).takeWhile { it != 'x' }.toInt()
        val repeatCount = getRepeatCount(currentString.substring(nextMarkerStart, nextMarkerEnd + 1))
        val substitution = processMarker(
            currentString.substring(nextMarkerEnd + 1, nextMarkerEnd + markerLength + 1), repeatCount
        )
        return decompress(substitution + currentString.substring(nextMarkerEnd + markerLength + 1), length + nextMarkerStart)
    }
